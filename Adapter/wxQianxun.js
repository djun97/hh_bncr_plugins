 ...
 * @author 小寒寒
 * @name wxQianxun
 * @origin framework-GGbon
 * @version 1.0.1
 * @description GGbon类千寻适配器
 * @adapter true
 * @public false
 * @disable false
 * @priority 2
...
module.exports = async () => {
  ...
  if (!Object.keys(ConfigDB.userConfig).length) {
    sysMethod.startOutLogs('未启用GGbon类千寻适配器,退出.');
    return;
  }
  if (!ConfigDB.userConfig.enable) return sysMethod.startOutLogs('未启用GGbon类千寻适配器 退出.');
  let QianxunUrl = ConfigDB.userConfig.sendUrl;
  if (!QianxunUrl) return console.log('GGbon:配置文件未设置sendUrl');
  ...
  /**向/api/系统路由中添加路由 */
  router.get('/api/bot/Qianxun', (req, res) => res.send({ msg: '这是Bncr GGbon类千寻适配器 Api接口，你的get请求测试正常~，请用post交互数据' }));
